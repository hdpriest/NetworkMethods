...
sub _RFrameToPNG {
	...
	if($type eq "line"){
		...
		$cmd="ggplot(data=DF,aes(x=Sample,y=Expression,group=Sample)) + geom_boxplot(stat = \"boxplot\", position = \"dodge\" ) + theme(axis.text.x = element_text(angle=90,hjust=1)) + ggtitle(\"$title\") + xlab(\"$xlab\") + ylab(\"$ylab\") + xlim(\"".join("\",\"",@header)."\") + ylim(0,8)";
	}
	$R->send($cmd);
	my $result = $R->read();
	warn $result."\n";
